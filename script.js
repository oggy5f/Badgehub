...
// Small helper: query element safely
...
// Escape text so SVG/HTML not break
...
function makeBadgeSVG(displayName, badgeType) {
  ...
  // FULL SVG WITH BACKTICKS (VERY IMPORTANT)
  return (<svg class="badge" xmlns="http://www.w3.org/2000/svg" width="720" height="360" viewBox="0 0 720 360">
    ...
    <text x="220" y="190" font-size="24" fill="#fff">
      ${escapeXml(badgeType)}
    </text>

    <text x="220" y="230" font-size="16" fill="#cfe0ff">
      ${escapeXml(date)}
    </text>
  </svg>).trim();
}
